...
| Puerto | R/W | Qué es |
|---|---|---|
| **0x2F** | R | **Versión del core** (`FPGA_VERSION` en `top.v`). Formato `0x1X` = 1.X, `0x20` = 2.0. El menú lo compara con la versión del pack y avisa si no cuadran. Sin guarda: se lee siempre |
...
| **0x41** | R/W | Con el dispositivo 8: **turbo Panasonic**. Bit 0 = 0 turbo 5,37 MHz, 1 = 3,58. `OUT &H41,128` activa, `OUT &H41,129` desactiva; `INP(&H41)` lee el estado |
| **0x42 – 0x45** | R/W | Registros de **configuración goauld** (con el dispositivo 0x48): slot 1, segundo SCC, scanlines, estéreo, sprites 8/línea, boot turbo, menú al arrancar. Los toca el menú de Ajustes; el 0x45 es el que persiste en la flash |
...
| **0x12** | — | Reservado (decodificado en `top.v`; sin uso en la v2.0) |
...
| **0xF2** | — | Decodificado en `top.v` (heredado) |
| **0xFF** | — | Idem |
