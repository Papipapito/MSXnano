...
wire [7:0] psg_joy_data_raw = (!psg_reg15_joy_sel[0]) ? joy0_goauld_msx
                                                      : psg_port2_q;
// CASIN (bit7 del PortA del PSG) en reposo ALTO, como un MSX real sin senal de
// cinta. La cinta virtual se retiro el 04/09/2026 con el proyecto que la
// alimentaba (el C6 dejo de convertir TSX y el menu perdio la tecla T).
wire [7:0]  psg_joy_data = {1'b1, psg_joy_data_raw[6:0]};
...
// Diagnostico del raton (puerto 0x2B), portado del 0x2E del MSXimus.
...
`endif
    ...
    // ===== Turbo mode (control: OUT &H41 software + Boot Turbo del menu) =====
    // Default turbo=0 -> M1 wait active -> ~100% real-MSX speed (3.58MHz behaviour).
    // v1.9c: SIN atajo de tecla (F11 quitado de ambos teclados). turbo=1 switches
...
`endif 
    ...
    `endif
        ...
        .maxspr_n    (~config_enable_8sprites),
...
`ifdef ENABLE_CONFIG
    ...
    assign config_enable_16_9 = 1'b0;  // 16:9 por AVI eliminado; fijo 4:3
    wire config_enable_8sprites;
    assign config_enable_8sprites = config2_ff[4];  // bit4: 8 sprites/linea (anti-parpadeo screen2 MSX1)
...
//    );
    ...
    assign led[4] = ~sd_busy_w;                  // actividad de la SD
